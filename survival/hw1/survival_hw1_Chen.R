#install.packages(c("muhaz","survminer"))
library(survival)
library(survminer)
library(muhaz)
library(dplyr)
...
# what percentage of pumps are in each failure
table(katrina$reason)/dim(katrina)[1]
...
#kat_haz_r <- with(katrina[katrina$reason!=0,], kphaz.fit(hour2, ifelse(survive==0,1,0),reason))

# plotting hazard function
kphaz.plot(kat_haz, main = "Figure 3: Hazard Function of the 48-hour Hurricane Period")

title(xlab="Hours")
#kphaz.plot(kat_haz_r, main = "hazard function")

# try ggplot for hazard plot
haz_data = data.frame(time=kat_haz$time,haz=kat_haz$haz)
haz_plot = ggplot(haz_data,aes(time,haz)) +
  geom_line()
