...
# age
visreg(fit, "age",
  xlab = "age", ylab = "partial residuals", gg = TRUE,
  band = FALSE
) +
  geom_smooth(col = "red", fill = "red") + theme_bw()
# prior convictions
visreg(fit, "prio",
  xlab = "#prior convictions", ylab = "partial residuals",
  gg = TRUE, band = FALSE
) +
  geom_smooth(col = "red", fill = "red") + theme_bw()
...
ggsurvplot(survfit(fit_strat),
  data = recid, fun = "cloglog",
...
newdata2 <- data.frame(
  fin = c("0", "1"), age = 30, race = "0", wexp = "0",
  mar = "0", paro = "0", prio = 4
)
ggsurvplot(survfit(fit, newdata2),
  data = newdata2,
  palette = c("black", "purple"), legend.labs = c("no", "yes"),
...
# load new datasets
recid_long <- read.csv("survival/data/recid_long.csv", header = TRUE)
fit_long <- coxph(Surv(start, stop, arrested == 1) ~ fin + age + race + wexp   mar + paro + prio + employed, data = recid_long)
summary(fit_long)
# use previous week instead
recid_lag <- read.csv("survival/data/recid_lag.csv", header = TRUE)
