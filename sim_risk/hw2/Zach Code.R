...
for (ii in 1:10000){
  ...
  decline_rate = runif(n=2, min=0.15, max=0.32)
  ...
  for (jj in 2019:2033){
    ...
    low = price_proj$Low_Price[which(price_proj$Year == jj)]
    mean = price_proj$Reference_Price[which(price_proj$Year == jj)]
    oil_price = rtriangle(n=1, a=low, b=high, c=mean) # obtain price of oil from price sheet in excel
